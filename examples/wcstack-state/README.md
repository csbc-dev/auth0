...
The fetch is handled by the official `<auth0-config>` tag and `attr.*` bindings — no manual `fetch`, `setAttribute`, or `customElements.whenDefined` plumbing in the page.
...
2. (Optional) Open [`index.html`](index.html) and adjust the two server URLs if your server is not at `http://localhost:3000`: the `<auth0-config src="…/auth-config">` endpoint and the `<script src="…/_shared/appCoreFacade.auto.js">` tag that loads the payload facade definition. Both point at the same example server.
...
[t1] <auth0-config src="http://localhost:3000/auth-config"> loads
     → state.config.domain / .clientId / ... assigned
...
- **Server-config-discovery via `<auth0-config>` + `attr.*`**: fetch happens inside the official config element; wcstack/state propagates its bindable outputs into state, then state→attribute changes onto `<auth0-gate>` automatically. No imperative `fetch`, `customElements.whenDefined()` / dynamic `import()` dance — `/auto` script tags load normally and the element waits for attributes to arrive.
