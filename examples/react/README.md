...
- `<app-core-facade>` is a schema-only custom element registered via `defineAppCoreFacade()` from `shared/appCoreFacade.js` (whose `static wcBindable` schema is the `appCoreDeclaration` shared with the server-side `AppCore` in `shared/appCore.js`); the session installs `count` / `connectedUser` mirrors and `increment` / `decrement` / `reset` command forwarders on it.
...
2. It synchronously installs the command forwarders on the child.
