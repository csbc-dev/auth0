# auth0-gate Remote CSBC Specification

## 1. Overview

Adapt `auth0-gate` to the remote **CSBC (Core/Shell Bindable Component)** architecture. Map the inherent two-layer structure of the Auth0 authentication flow — **"browser-dependent parts"** and **"server-completable parts"** — precisely onto the remote CSBC **Shell/Core boundary**.
...
### Fundamental Difference from s3-uploader

s3-uploader follows the pattern "Shell connects to Core and delegates operations." The Shell acts as a proxy for the Core and subscribes to Core property changes.

auth0-gate remote follows the pattern **"Cores are constructed only after authentication succeeds."** The Auth0 Shell functions as a **gatekeeper** to other Cores, responsible for establishing an authenticated WebSocket connection. The Core itself contains no authentication logic — **it already holds an authenticated user context at the time of construction**.
...
async connect(url: string): Promise<ClientTransport> {
  ...
  this._url = url;
  const ws = new WebSocket(url, [`auth0-gate.bearer.${token}`]);
  this._ws = ws;
...
- For the reference `UserCore`, wire it as `(core, user) => core.updateUser(user)`. `updateUser` dispatches `auth0-gate-user:permissions-changed` / `roles-changed` / `user-changed` only when the corresponding field actually changed, so the client sees exactly the deltas the new token implies and nothing more. The `auth0-gate-user:` prefix separates UserCore's server-side events from `AuthCore`'s `auth0-gate:user-changed` (which carries the client-side ID-token profile).
...
The `proxy` argument is required because Strategy B needs `proxy.reconnect(newTransport)` to swap the proxy onto the rebuilt connection (§3.4.2). Callers using `<auth0-session>` can pass `session.proxy` once `session.ready` is true.
...
async reconnect(): Promise<ClientTransport> {
  ...
  const ws = new WebSocket(this._url, [`auth0-gate.bearer.${token}`]);
  this._ws = ws;
  ws.addEventListener("close", () => {
    if (this._ws === ws) this._setConnected(false);
  });
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      ws.addEventListener("error", () => reject(
        new Error(`[@csbc-dev/auth0] WebSocket reconnection failed: ${this._url}`)
      ), { once: true });
    });
  } catch (err) {
    this._setConnected(false);
...
### 3.5 HTMLElement Wrapper (`<auth0-gate>`)
...
|---------------|----------------------------------------|--------------------------------|
| `remote-url`  | WebSocket URL of the Core server. **Setting this does NOT make `<auth0-gate>` auto-connect** — it only provides the default URL used by `connect()` / `<auth0-session>`. See §3.7. For mode inference, only a **non-empty** value implies `mode="remote"`; `remote-url=""` is treated as unset so a transient empty binding does not flip the element into a broken remote state (use explicit `mode="remote"` to override). | `wss://api.example.com/CSBC` |
...
`<auth0-gate>` does not open a WebSocket on its own. A connection is only created when something calls `authShell.connect(url)` (or `authEl.connect(url)`). There are exactly **two supported patterns**, and they MUST NOT be combined on the same `<auth0-gate>` instance:
...
**Why they must not be combined.** `AuthShell.connect()` unconditionally calls `_closeWebSocket()` at its start (so a failed handshake cannot leak a previous socket). This means a second `connect()` call from the application closes the WebSocket that `<auth0-session>` just opened, leaving the session's `RemoteCoreProxy` bound to a transport that will never deliver another frame. The session keeps `ready=true` but property updates silently stop. There is no way for the session element to "take over" a transport it did not create. **Pick one pattern per `<auth0-gate>` instance.**

**Enforcement.** `<auth0-session>` fails fast: if `authEl.connected === true` at the point where the session would have called `connect()` itself, the session sets `error` to a message describing the conflict and does not build a proxy. This surfaces the mistake immediately instead of producing a silently-dead session.
...
```typescript
import { verifyAuth0Token } from "@csbc-dev/auth0/server";
...
export class UserCore extends EventTarget {
  static wcBindable: IWcBindable = {
    ...
    version: 1,
    properties: [
      // `auth0-gate-user:` namespace disambiguates these events from
      // `AuthCore`'s `auth0-gate:user-changed` (client-side ID-token
...
Authorization in the remote CSBC architecture operates at two levels: **declarative** (metadata-driven, enforced before the command reaches the Core) and **imperative** (hand-written logic inside the Core method). Both are needed; neither alone is sufficient.
...
4. Establish WebSocket connection
   └─ Call AuthShell.connect(url) — either directly or via <auth0-session> (§3.7)
      └─ getTokenSilently() to obtain access token
      └─ WebSocket(url, ["auth0-gate.bearer.{token}"])
...
| Token in DevTools component tree | Token not a public property on `<auth0-gate>` — won't show in React DevTools, Vue Devtools property panels | **Effective** for casual inspection. The Auth0 SDK still holds it internally. |
...
|--------|---------|-----------------|
| **XSS with arbitrary JS execution** | If an attacker can run `await auth0Client.getTokenSilently()`, they get the token regardless of whether auth0-gate exposes it. The Auth0 SPA SDK holds the token in a JS-reachable closure/cache. `cacheLocation: "memory"` does NOT mean "inaccessible to JS" — it means "not in `localStorage`". | **Prevent XSS in the first place.** CSP, input sanitization, dependency auditing. Once XSS is achieved, all in-browser secrets are compromised — this is a fundamental browser security boundary. |
...
3. Client opens WebSocket with the ticket instead of the token:
   new WebSocket(url, ["auth0-gate.ticket.{ticket}"])
...
const AppCoreDeclaration = {
  ...
  properties: [
    { name: "currentUser", event: "auth0-gate-user:user-changed" },
    { name: "permissions", event: "auth0-gate-user:permissions-changed" },
    { name: "objects",     event: "s3-uploader:objects-changed" },
  ],
  commands: [
    { name: "requestUpload", async: true },
...
function App() {
  // --- 1. Subscribe to <auth0-gate> element via useWcBindable ---
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const transport = await el.connect("wss://api.example.com/CSBC");
      if (cancelled) return;
  ...
  if (!auth.authenticated) {
    return (
      <>
        {/* auth0-gate element (hidden, manages auth state) */}
        <auth0-gate
          ref={authRef}
          domain="your-tenant.auth0.com"
...
console.log("CSBC Auth0 server listening on :3000");
```
...
const transport = await authShell.connect("wss://api.example.com/CSBC");
const proxy = createRemoteCoreProxy(AppCoreDeclaration, transport);
...
## 12. Comparison with s3-uploader

| Aspect              | s3-uploader (remote)                           | auth0-gate (remote)                              |
|---------------------|--------------------------------------------|--------------------------------------------------|
...
| Item                          | Options                                              | Recommended            |
|-------------------------------|------------------------------------------------------|------------------------|
| Declarative authorization     | `auth0-gate/server` middleware (§5.3.1) / native `RemoteShellProxy` hook | Middleware in `auth0-gate/server` (initial); propose `onBeforeCommand` hook upstream to `@wc-bindable/remote` later |
...
| Health check endpoint         | Load balancers need a way to verify the server is healthy without authenticating. | Reserve a subprotocol `auth0-gate.healthcheck` that the server responds to with an immediate close (code 1000). Alternatively, expose a separate HTTP `/health` endpoint alongside the WebSocket server. Recommendation: HTTP `/health` is simpler and more conventional. |
