...
describe("AuthCore", () => {
  ...
  describe("initialize", () => {
    it("domain未指定時にエラーをスローする", () => {
      ...
      expect(() => core.initialize({ domain: "", clientId: "id" })).toThrow(
        "[@csbc-dev/auth0] domain is required."
      );
    });
    ...
    it("clientId未指定時にエラーをスローする", () => {
      ...
      expect(() => core.initialize({ domain: "test.auth0.com", clientId: "" })).toThrow(
        "[@csbc-dev/auth0] clientId is required."
      );
    });
    ...
    it("Auth0クライアントを初期化できる（未認証）", async () => {
      ...
      const core = new AuthCore();
      const events: string[] = [];
      core.addEventListener("auth0-gate:loading-changed", () => events.push("loading"));
      core.addEventListener("auth0-gate:authenticated-changed", () => events.push("authenticated"));
    ...
    it("_syncState トークン取得失敗時に auth0-gate:error が発火する (I-004)", async () => {
      // Regression: previously `_syncState`'s catch block was bare
      // (`catch (_e)`) so no error event was dispatched. Subscribers
      // binding through `auth0-gate:error` would never learn the token
      ...
      core.addEventListener("auth0-gate:error", (e) => {
    ...
    it("初期化時にerrorクリアイベントが発火する", async () => {
      ...
      core.addEventListener("auth0-gate:error", (e: Event) => {
        errorEvents.push((e as CustomEvent).detail);
      });
  ...
  describe("target指定", () => {
    ...
    it("target指定時はイベントがtargetにディスパッチされる", async () => {
      ...
      core.addEventListener("auth0-gate:loading-changed", () => coreEvents.push("loading"));
      target.addEventListener("auth0-gate:loading-changed", () => targetEvents.push("loading"));
  ...
  describe("login", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.login()).rejects.toThrow("[@csbc-dev/auth0] Auth0 client is not initialized.");
    });
    ...
    it("login()でloadingがtrueになりerrorがクリアされる", async () => {
      ...
      core.addEventListener("auth0-gate:loading-changed", (e: Event) => {
  ...
  describe("loginWithPopup", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.loginWithPopup()).rejects.toThrow("[@csbc-dev/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("logout", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.logout()).rejects.toThrow("[@csbc-dev/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("getToken", () => {
    it("クライアント未初期化時にエラーをスローする", async () => {
      const core = new AuthCore();
      await expect(core.getToken()).rejects.toThrow("[@csbc-dev/auth0] Auth0 client is not initialized.");
    });
  ...
  describe("fetchToken / fetchFreshToken / commitToken", () => {
    it("fetchToken throws when called before initialize", async () => {
      ...
      await expect(core.fetchToken()).rejects.toThrow(
        "[@csbc-dev/auth0] Auth0 client is not initialized.",
      );
    });
    ...
    it("commitToken updates token and dispatches token-changed", () => {
      ...
      core.addEventListener("auth0-gate:token-changed", (e: Event) => {
