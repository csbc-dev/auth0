...
/**
 * <auth0-logout> — declarative logout button.
 * Finds the parent or referenced <auth0-gate> element and calls logout().
 *
 * Usage:
 *   <auth0-logout target="auth-id">ログアウト</auth0-logout>
 *   <auth0-logout return-to="/">ログアウト</auth0-logout>
 */
export class AuthLogout extends HTMLElement {
  ...
  private _handleClick = (event: Event): void => {
    ...
    if (!authElement) {
      ...
      // bug (mistyped target ID, no <auth0-gate> in the document) — the
    ...
    // target <auth0-gate> has finished initialising, so AuthCore's
    ...
    // through `authEl.error` / `auth0-gate:error` as before, so
