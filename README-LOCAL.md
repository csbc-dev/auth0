...
In the **CSBC (Core/Shell Bindable Component)** taxonomy this is the **Case A** shape: the Core remains in the browser because the Auth0 SPA SDK, redirect callback handling, and token lifecycle are browser-anchored.
...
`@csbc-dev/auth0` moves authentication logic into a reusable component and exposes the result as bindable state.
...
```bash
npm install @csbc-dev/auth0
```

### Peer dependency

`@csbc-dev/auth0` requires the Auth0 SPA SDK:
...
These properties represent the current authentication state and are the main CSBC surface:
...
`@csbc-dev/auth0` follows an **observable error** contract: errors raised by the Auth0 SDK surface as state you bind to, not as rejected promises. This keeps UI code declarative — you render from `error` / `loading`, rather than wrapping every call in `try / catch`.
...
`@csbc-dev/auth0` follows the CSBC architecture.
...
"Headless" here means **without the Shell element**, not without a browser. `AuthCore` depends on `@auth0/auth0-spa-js`, `globalThis.location`, and `globalThis.history`, so it requires a browser-like environment — it will not run under bare Node.js / Deno / Cloudflare Workers. The CSBC pattern is runtime-agnostic; this particular Core implementation is bound to the browser by its Auth0 SDK and redirect-callback dependencies. See §Headless Usage and the Design Notes at the end of this file for the full caveat.
...
```typescript
import { AuthCore } from "@csbc-dev/auth0";
...
Since `<auth0-gate>` is CSBC + `wc-bindable-protocol`, it works with any framework through thin adapters from `@wc-bindable/*`.
...
import type { AuthValues } from "@csbc-dev/auth0";
